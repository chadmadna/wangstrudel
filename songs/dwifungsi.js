...
let drumsPat = {
  ...
  fillIntro: stack(
    ...
    s(`~!3 cp`).vel(.4),
    s("cfx").slow(2).delay(.4).dt(.3).dfb(.7).gain(0.5),
  ),
  fillVerse: stack(
    s(`bd*4`),
...
let DFILL1 = drumsPat.fillIntro.gain(slider(0.8666,0,1.4))
let DFILL2 = drumsPat.fillVerse.gain(slider(0.8666,0,1.4))
/*
...
let GUITAR = (pat) => pat
  ...
  // .fm(perlin.range(24, 35).div(10)).fmh(rand.range(7001, 7005).div(1000)).fm2(perlin.range(20, 30).div(10)).fmh2(2.01).fmdec2(2).fmsus(.3)
  .gain(0.3)
...
let BASSGTR = (pat) => pat
  .s("gm_slap_bass_2").distort("2.6:.2:sinefold").hpf(70).hpq(1)
  .gain(0.8)
...
let LEGTR = note(`[7]@3 [11]@11 [12]@2 [11]@3 [9]@11 [8]@2`).slow(4)
...
let LEAD1 = note(`[[9 ~]!2 ~ 9 10b 9 10b 6 [8 ~]!2 ~ 7 ~ 7 ~ 7b]!3 [[9 ~]!2 ~ 9 10b 9 10b 6 7@8]`)
  .scale("C:phrygian").slow(8).trans(-24).s("<wanglead:0!7 wanglead:1>").legato(1.1).rel(0.1).lpf(saw.range(400, 6000).seg(64).slow(8)).lpq(10)
  .gain(.75)

let LEAD2 = note(`[[9 ~]!2 ~ 9 10b 9 10b 6 [8 ~]!2 ~ 7 ~ 7 ~ 7b]!3 [[9 ~]!2 ~ 9 10b 9 10b 6 7@8]`)
  .scale("C:phrygian").slow(8).trans(-24).s("<wanglead:0!7 wanglead:1>").legato(1.1).rel(0.1)
  .gain(.8)
...
let SNARERUSH = s("~!3 [sd!32]").slow(8)
  .phaser(.8).gain(saw.slow(2).range(0.3, 1.3))
  .postgain(slider(1,0,1))

let WANGNOISE = s("wangnoise").rel(.7).hell("{0!3 9@2 6!3 11@2 13!3 5@2 4@1}%9".div(32), 4)
  .fast(2).jux(press).o(1).delay(.3).room(.3)
  .gain(slider(0.8498, 0, 1.4))

/**********************************
  *            MIXER!!            *
  **********************************/
$MAINLOOP: stack(
  DRUMS(drumsPat.intro),
  // DFILL2,
  BASSGTR(bassGtrPat.intro),
  // arrange([4, note(`~`)], [4, GUITAR(guitarPat.intro)]),
  // GUITAR(guitarPat.verse),
  // KEYS(keysPat.main),
  // LEGTR,
  CRASH,
  // RISER,
  // SNARERUSH,
  // WANGNOISE,
).compressor("-12:5:.9:.04:.05").postgain(.8)

$CHORUS: stack(
  // stack(DRUMS(drumsPat.chorus1), BASSGTR(bassGtrPat.chorus1), KEYS(keysPat.chorus1), LEAD1, CRASH),
  // stack(DRUMS(drumsPat.chorus2), BASSGTR(bassGtrPat.chorus2), KEYS(keysPat.chorus2), LEAD2, CRASH),
  // stack(DRUMS(drumsPat.chorus3), BASSGTR(bassGtrPat.chorus2), KEYS(keysPat.chorus2), LEAD2, CRASH),
  // stack(DRUMS(drumsPat.chorusOut), BASSGTR(bassGtrPat.chorusOut), LEADOUT)
)
  // .chebyshev(slider(0, 0, 0.5, 0.02))
  .compressor("-13:5:.9:.04:.05").postgain(.8)
