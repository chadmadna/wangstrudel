...
let mask1 = osc(trig.range(5, 30), 0, 0)
	.kaleid(99)
	.modulateScale(osc(trig.range(5, 40), 0, 0.3)
		.kaleid(4), 1)
	.diff(fade)
	.luma()
...
let scene1 = () => osc(100, 10, .4).mask(mask1).mult(fade)
	.out(o1)

let scene2 = () => osc(100, 10, .4).mask(mask2).mult(fade)
	.out(o1)

let scene3 = () => src(s1)
	.scale(.8, 1.3)
	.mask(shape(99, .6, .2).mult(fade))
	.scale(trig)
	.out(o1)
...
midi.input(0).channel(9)
	.onNote('*', ({ note, velocity, channel }) => {
		if ([36, 57, 41, 43].includes(note)) { scene1(); return; }
  		if ([38, 46, 49].includes(note)) { scene2(); return; }
	})

// midi in from DAW
midi.input(2).channel(0)
	.onNote('*', ({ note, velocity, channel }) => {
		if ([36, 57, 41, 43].includes(note)) { scene1(); return; }
  		if ([38, 46, 49].includes(note)) { scene2(); return; }
	})

midi.input(0).channel(9).onNote(53, () => scene3())
midi.input(0).channel(9).onNote(55, () => scene4())
